...
def generate():
    try:
        ...
        if qr_type == "text" and data.get("_iosCompat"):
            ...
            # Get the host from the request to build a full URL
            host = request.host
            scheme = request.scheme
            encoded = quote(text_content, safe='')
            data = {"url": f"{scheme}://{host}/t/{encoded}"}
            qr_type = "url"
...
@app.route("/t/<path:text>")
def show_text(text):
    """Display QR code text content in a clean page — used for iOS compatibility."""
...
<style>
  ...
  .content{{font-size:1.1rem;line-height:1.7;white-space:pre-wrap;word-break:break-word;}}
